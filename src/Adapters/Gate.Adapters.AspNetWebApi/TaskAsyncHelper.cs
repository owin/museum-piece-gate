...
namespace Gate.Adapters.AspNetWebApi
{
    /// <summary>
    /// Copied from 
    /// https://raw.github.com/SignalR/SignalR/master/SignalR/TaskAsyncHelper.cs
    /// and edited
    /// </summary>
    internal static class TaskAsyncHelper
    {
        ...
        {
            return task.Catch(ex => Trace.TraceError("Unhandled exception: {0}", ex));
        }
        ...
        public static TTask Catch<TTask>(this TTask task, Action<Exception> handler) where TTask : Task
        {
            if (task != null && task.Status != TaskStatus.RanToCompletion)
            {
                task.ContinueWith(innerTask =>
                {
                    try
                    {
                        handler(innerTask.Exception);
                    }
                    catch (Exception ex)
                    {
                        Trace.TraceError("Fault thrown from exception handler: {0}", ex);
                    }
